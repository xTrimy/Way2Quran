<div>
    <button id="surah_love_button" data-surah="{{$surah->id}}" data-reciter="{{ $reciter->id }}" wire:click="$emitSelf('love')" class="rounded-full mr-4 mb-2 text-neutral-400 flex items-center py-1 pl-4 pr-8 text-lg border border-neutral-400">
        @if($added)
            <i class="las dark:text-green-500 text-green-600 transition-colors la-heart text-2xl mr-4"></i> Added
        @else
    ...
    <script>
        window.addEventListener('loveAdded',function(event){
            ...
            if(added){
                document.getElementById('surah_love_button').innerHTML = '<i class="las dark:text-green-500 text-green-600 transition-colors la-heart text-2xl mr-4"></i> Added';
            }else{
